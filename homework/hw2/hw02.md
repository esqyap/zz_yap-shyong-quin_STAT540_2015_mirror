...
![](hw02_files/figure-html/unnamed-chunk-50-1.png) 
...
The `edgeR` method is more robust to reduced sequencing depth because 18% of DE genes identified in the DEA of the high sequencing depth dataset were retained in the DEA results of the low sequencing depth dataset. In contrast to the `edgeR` method, no DE genes were retained using the `voom+limma` method when sequencing depth was reduced. 
