...
pr.err <- matrix(-1, nfold*nruns, nmethod, 
                 dimnames=list(paste0("Fold", 
                                      ...
                                      c("1NN", "5NN", "10NN", 
                                        "15NN", "LDA",
                                        "Logit", "SVM", "10NN-p.value")))
```

Modify the loop for feature selection+modeling so that it performs 100 runs of CV.
...
for (run in 1:nruns){
  ...
  for(i in 1:nfold){
    ...
    yhat.knn <- knn(train=t(traindat.fold.pval), 
                      ...
                      k = 10)
    pr.err[i+((run-1)*nfold),"10NN-p.value"]<- mean(testclass.fold != yhat.knn)
    } #end of CV loop
  } # end of 100 runs
```
